...
CREATE TABLE IF NOT EXISTS `oc_pizza`.`article_quantity` (
  ...
  `quantity` INT NOT NULL,
  PRIMARY KEY (`order_id`, `client_id`, `article`),
  INDEX `article_idx` (`article` ASC) ,
  INDEX `FK_article_quantity_order_idx` (`client_id` ASC) ,
  ...
  CONSTRAINT `FK_article_quantity_article`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `FK_article_quantity_client_id`
    FOREIGN KEY (`client_id`)
    REFERENCES `oc_pizza`.`order` (`client_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
